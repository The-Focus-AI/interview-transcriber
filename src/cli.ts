...
program
  ...
  .option(
    "-t, --temp-dir <path>",
    "Temporary directory for processing (default: ./temp)"
  )
  .option(
    "-c, --chunk-duration <seconds>",
    "Duration of each chunk in seconds (default: 600)",
    parseInt
  )
  .option(
    "--concurrency <number>",
    "Number of chunks to process in parallel during transcription (default: 5)",
    parseInt
  )
  .option("-k, --keep-chunks", "Keep temporary audio chunks after processing")
  .option(
    "-s, --save-temp-files",
    "Keep all temporary files including raw audio, downsampled audio, chunks, and intermediate files"
  )
  ...
  .action(async () => {
    ...
    try {
      ...
      const testOptions: ProcessingOptions = {
        ...
        concurrency: 5, // Test with 5 parallel chunks
      };
