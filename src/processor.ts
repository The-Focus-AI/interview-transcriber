  ...
  async processAudio(options: ProcessingOptions): Promise<FinalOutput> {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Cannot continue without audio: ${(error as Error).message}`
        );
      }
      ...
      try {
        chunks = await this.chunker.splitAudio(
          downsampledPath,
          chunksDir,
          chunkDuration
        );
        ...
        throw new Error(
          `Cannot continue without audio chunks: ${(error as Error).message}`
        );
      }

      // Step 5: Transcribe all chunks
      console.log("\n🎯 STEP 5: Transcribing audio chunks...");
      const chunkTranscriptions = await this.transcriber.transcribeAllChunks(
        chunks,
        options.concurrency || 5
      );
      const successfulChunks = chunkTranscriptions.filter(
        (chunk) =>
          chunk.segments.length > 0 &&
          !(
            chunk.segments.length === 1 &&
            chunk.segments[0].text.startsWith("Error transcribing")
          )
      );
      ...
      if (successfulChunks.length > 0) {
        ...
        console.log(
          `✅ Successfully transcribed ${successfulChunks.length}/${chunks.length} chunks`
        );
      } else {
        console.error("❌ All transcription attempts failed");
        throw new Error(
          "Cannot continue without any successful transcriptions"
        );
      }
      ...
      if (options.saveAllTempFiles) {
        ...
        const chunkTranscriptionsPath = path.join(
          runTempDir,
          "chunk_transcriptions.json"
        );
        await fs.writeFile(
      ...
      if (options.saveAllTempFiles) {
        ...
        const mergedSegmentsPath = path.join(
          runTempDir,
        ...
        const identifiedSegmentsPath = path.join(
          runTempDir,
      ...
      try {
        highlights = await this.highlightsExtractor.extractHighlights(
          identifiedSegments
        );
        stepStatus.highlights = true;
      } catch (error) {
        console.error(
          `❌ Highlight extraction failed: ${(error as Error).message}`
        );
        highlights = [
          "Highlight extraction failed. See transcript for details.",
        ];
      }

      try {
        summary = await this.highlightsExtractor.generateSummary(
          identifiedSegments
        );
      ...
      if (options.outputPath && typeof options.outputPath === "string") {
        ...
        outputPath = path.join(
          outputDir,
          generateOutputFilename(options.url, "json")
        );
      }
      ...
      await this.outputBuilder.saveTranscriptAsText(
        identifiedSegments,
        textPath
      );
      ...
      if (Object.values(stepStatus).every((status) => status === true)) {
        ...
        console.log(
          "Some steps encountered errors but the process was able to continue."
        );
      }
